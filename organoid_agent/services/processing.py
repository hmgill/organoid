...
def create_filled_mask(binary_mask: np.ndarray, original_image: Image.Image, smooth: bool = True) -> Image.Image:
    ...
    # Create a FILLED mask for the contour region
    ...
    cv2.drawContours(
        ...
        thickness=-1  # Negative = filled (this fills interior holes!)
    )
    ...
    # Convert background-removed image to numpy array (RGB)
    bg_removed_array = np.array(original_image.convert('RGB'))
    
    # Start with white background
    result = np.ones_like(bg_removed_array) * 255
    
    # Copy pixels from BACKGROUND-REMOVED image where filled contour mask is active
...
def execute_sam3_segmentation(image_path: str, point_x: int, point_y: int) -> str:
    ...
    try:
        ...
        binary_mask = (mask_probs > 0.05).numpy().astype('uint8') * 255
